...
export class Attire {
  ...
  getwhen(black: number, blue: number, green: number, red: number) {
    const rule = this.attire.rules.find((rule) =>
      this.ruleMatchWith(rule, black, blue, green, red)
    );
    if (!rule) {
      return { image: "", text: "" };
    }
  ...
  ruleMatchWith(
    ...
    return (
      this.match(rule.when.black, black) &&
  ...
  match(when: number | string, amount: number) {
    return typeof when === "number"
      ? when === amount
      : this.matchRegex(when as string, amount);
  }
