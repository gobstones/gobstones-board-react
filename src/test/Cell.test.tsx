...
function getCell() {
    return render(<Cell isHeader={false} content={cellInfo} addBlue={blueClickedFunction}
                        removeBlue={blueRemovedFunction} addBlack={blackClickedFunction}
                        removeBlack={blackRemovedFunction} addRed={redClickedFunction}
                        removeRed={redRemovedFunction} addGreen={greenClickedFunction}
                        removeGreen={greenRemovedFunction} attire={{image: '', text: ''}}/>)
}

describe('<Cell/>', () => {
    it('is created with a cell info content', () => {
        let documentBody = getCell();
        expect(documentBody.getByRole('blue-stone')).toHaveTextContent(cellInfo.a.toString());
        expect(documentBody.getByRole('red-stone')).toHaveTextContent(cellInfo.r.toString());
        expect(documentBody.getByRole('green-stone')).toHaveTextContent(cellInfo.v.toString());
        expect(documentBody.getByRole('black-stone')).toHaveTextContent(cellInfo.n.toString());
    });
